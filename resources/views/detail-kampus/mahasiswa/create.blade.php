...
@section('content')
    <form action="{{ route('detail-kampus.mahasiswa.store', ['kampus' => $kampus->id]) }}" method="POST">
        <div class="card">
                ...
                <div class="mb-3">
                    @foreach ($itemBayars as $index => $group)
                        @foreach ($group as $loop => $itemBayar)
                            ...
                            @endif
                            <option value="{{ $itemBayar->id }}" {{ old('item_bayar_selected.*') == $itemBayar->id ? 'selected' : '' }}>x{{ $itemBayar->jumlah_angsuran }}</option>
                        @endforeach
                        </select>
