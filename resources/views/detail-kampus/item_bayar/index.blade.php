...
@section('content')
    <div class="d-flex justify-content-between mb-2">
        <a href="{{ route('detail-kampus.item-bayar.create', ['kampus' => $kampus->id]) }}" class="btn btn-primary">Tambah</a>
    </div>
                    ...
                    <tr>
                        ...
                        <td>
                            @if ($item_bayar->jenis == 'open')
                                &infin;
                            @else
                                x{{ $item_bayar->jumlah_angsuran }}
                        </td>
                @endif
                <td>
                    <div class="d-flex gap-2">
                        <a href="{{ route('detail-kampus.item-bayar.edit', ['kampus' => $kampus->id, 'item-bayar' => $item_bayar->id]) }}" class="btn btn-warning btn-sm">Edit</a>
                        <form action="{{ route('detail-kampus.item-bayar.destroy', ['kampus' => $kampus->id, 'item-bayar' => $item_bayar->id]) }}" method="post">
                            @csrf
                            @method('DELETE')
