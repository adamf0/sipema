...
@section('page-title', 'Edit JAdwal Ulang Tagihan : ' . $jadwal_ulang->id_mahasiswa)

@section('content')
    <form action="{{ route('kampus.jadwal_ulang.update',['jadwal_ulang'=>$jadwal_ulang->id]) }}" method="POST" enctype="multipart/form-data">
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div class="mb-3">
                    ...
                    <select name="id_mahasiswa" id="id_mahasiswa" class="form-select @error('id_mahasiswa') is-invalid @enderror">
                        ...
                        @foreach ($mahasiswas as $mahasiswa)
                            <option value="{{ $mahasiswa->id }}" {{ old('id_mahasiswa',$jadwal_ulang->id_mahasiswa) == $mahasiswa->id ? 'selected' : '' }}>{{ $mahasiswa->nama_lengkap }}</option>
                        @endforeach
                    </select>
                    @error('id_mahasiswa')
                ...
                <div class="mb-3">
                    <label for="form-label">Dokumen Sebelumnya</label>
                    <br>
                    @if (Storage::disk('public')->exists('images/master/dokumen/' . $jadwal_ulang->dokumen))
                        <img width="100px" src="{{ asset('storage/images/master/dokumen/' . $jadwal_ulang->dokumen) }}" class="img-fluid">
                    @else
                ...
                <div class="mb-3">
                    ...
                    <input type="file" name="dokumen" value="{{ old('dokumen',$jadwal_ulang->dokumen) }}" class="form-control @if ($errors->any()) is-invalid @endif" id="dokumen" aria-describedby="dokumen" />
                    @if ($errors->any() && !$errors->has('dokumen'))
                        <div class="invalid-feedback">Terjadi Kesalahan pada Form,Dokuen harus di inputkan kembali</div>
                    @endif
                ...
                <div class="mb-3">
                    ...
                    <input type="date" name="tanggal_diundur" value="{{ old('tanggal_diundur',$jadwal_ulang->tanggal_diundur) }}" class="form-control @error('tanggal_diundur') is-invalid @enderror" id="tanggal_diundur" aria-describedby="tanggal_diundur" />
