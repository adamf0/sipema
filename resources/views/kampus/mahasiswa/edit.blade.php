...
@push('js')
    ...
    <script>
        $(document).ready(function() {
            ...
            var prodi = {{ old('prodi',$mahasiswa->id_prodi) }};
            var kelas = {{ old('prodi',$mahasiswa->id_kelas) }};
            var metode_belajar = {{ old('prodi',$mahasiswa->id_metode_belajar) }};
            var lulusan = {{ old('prodi',$mahasiswa->id_lulusan) }};
                    ...
                    callDataItems().then((data) => {
                        ...
                        if(data.length>0){
                            data.forEach((element,index) => {
                                ...
                                $("#item_bayar_selected_"+index).val(selected[index]).trigger('change');
                            });
                        }
                        else{
                            @foreach ($items as $index => $item)
