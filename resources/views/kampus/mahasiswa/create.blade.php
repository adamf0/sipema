...
@section('content')
    <form action="{{ route('kampus.mahasiswa.store') }}" method="POST">
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="nim" value="{{ old('nim') }}" class="form-control @error('nim') is-invalid @enderror" id="nim" aria-describedby="nim" />
                    @error('nim')
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="nama_lengkap" value="{{ old('nama_lengkap') }}" class="form-control @error('nama_lengkap') is-invalid @enderror" id="nama-lengkap-mahasiswa" aria-describedby="nama-lengkap-mahasiswa" />
                    @error('nama_lengkap')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="date" name="tanggal_lahir" value="{{ old('tanggal_lahir') }}" class="form-control @error('tanggal_lahir') is-invalid @enderror" id="tanggal-lahir" aria-describedby="tanggal-lahir" />
                    @error('tanggal_lahir')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select name="kelas" id="kelas" class="form-select @error('kelas') is-invalid @enderror">
                        ...
                        @foreach ($kelass as $kelas)
                            <option value="{{ $kelas->id }}" {{ old('kelas') == $kelas->id ? 'selected' : '' }}>{{ ucwords($kelas->nama) }}</option>
                        @endforeach
                    </select>
                    @error('kelas')
                        <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="mb-3">
                    ...
                    <select name="metode_belajar" id="metode_belajar" class="form-select @error('metode_belajar') is-invalid @enderror">
                        ...
                        @foreach ($metode_belajars as $metode_belajar)
                            <option value="{{ $metode_belajar->id }}" {{ old('metode_belajar') == $metode_belajar->id ? 'selected' : '' }}>{{ ucwords($metode_belajar->nama) }}</option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <select name="lulusan" id="lulusan" class="form-select @error('lulusan') is-invalid @enderror">
                        ...
                        @foreach ($lulusans as $lulusan)
                            <option value="{{ $lulusan->id }}" {{ old('lulusan') == $lulusan->id ? 'selected' : '' }}>{{ ucwords($lulusan->nama) }}</option>
                        @endforeach
                    </select>
                    @error('lulusan')
                        <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="mb-3">
                    ...
                    <input type="date" name="tanggal_pembayaran" value="{{ old('tanggal_pembayaran') }}" class="form-control @error('tanggal_pembayaran') is-invalid @enderror" id="tanggal-pembayaran" aria-describedby="tanggal-pembayaran" />
                    ...
                    callDataItems().then((data) => {
                        ...
                        if(data.length>0){
                            data.forEach((element,index) => {
                                ...
                                $("#item_bayar_selected_"+index).append("<option>-- Pilih Jumlah Angsuran "+element[0].item.nama+" --</option>");
