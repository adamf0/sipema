...
@section('page-title', 'Edit Master User : ' . $user->name)
...
@section('content')
    <form action="{{ route('master.user.update',['user'=>$user->id]) }}" method="POST">
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="nama" value="{{ old('nama',$user->name) }}" class="form-control @error('nama') is-invalid @enderror" id="nama" aria-describedby="nama" />
                    @error('nama')
                ...
                <div class="mb-3">
                    ...
                    <select name="role" id="role" class="form-select @error('role') is-invalid @enderror">
                        ...
                        @foreach ($roles as $role)
                            <option value="{{ $role->name }}" {{ old('role',$user->roles->pluck('name')->toArray()[0]) == $role->name ? 'selected' : '' }}>{{ ucwords($role->name) }}</option>
                        @endforeach
                    </select>
                    @error('role')
                        <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="mb-3 panel-kampus">
                    ...
                    <select name="kampus[]" id="kampus" class="form-select @error('kampus') is-invalid @enderror" multiple>
                        @foreach ($kampuss as $kampus)
                            @forelse ($user->user_kampus->pluck('id_kampus') as $item)
                                <option value="{{ $kampus->id }}" {{ old('kampus',$item) == $kampus->id ? 'selected' : '' }}>{{ ucwords($kampus->nama_kampus) }}</option>
                            @empty
